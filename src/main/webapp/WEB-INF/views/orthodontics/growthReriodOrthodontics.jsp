<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8" %>
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
<%@ taglib prefix="sec" uri="http://www.springframework.org/security/tags" %>

<link rel="stylesheet" href="<c:url value='/lib/slick/css/slick.min.css' />">
<link rel="stylesheet" href="<c:url value='/lib/slick/css/slick.theme.css' />">
<link rel="stylesheet" href="<c:url value="/css/orthodontics/growthReriodOrthodontics.css"/>" />

<div class="sub-contents grow">
    <h1 class="sub-page-title">성장기 교정</h1>
    <p class="sub-page-intro">GROWTH RERIOD ORTHODONTICS</p>

    <section class="section01 section-top-title section-top-title-left">
        <div class="image_box_left">
            <p class="img"></p>
        </div>
        <div class="max-layout-width text">
            <div>
                <h2 class="section-title">
                    청소년기 바람직한<br>
                    성장을 위한 치아교정<br>
                    <span class="section-title-color">
                        <strong>치열의 부조화는 미리<br>예방해야 합니다</strong>
                    </span>
                </h2>
            </div>
        </div>
        <div class="bg">
            <div class="max-layout-width">
                <div>
                    <p class="section-text">
                        성장기 청소년이 주걱턱이나,무턱 등의 골격적 문제를 보이는 경우 성장을 이용하여 골격 문제를 치료할 수 있습니다.
                        <strong>성장기 교정치료는 적절한 교정 치료 시기를 판단하는 것이 매우 중요</strong>합니다. 교정 치료 시기는 아이마다 다르기 때문에
                        <strong>주기적인 검진을 통해 성장 관찰 모니터링</strong>을 하면서 골격 문제,영구치 맹출 등의 증상에 따라 그 시기를 결정하는 것이 좋습니다.
                        우리 아이가 <strong>예쁜 미소와 건강한 치아와 외모에 대한 자신감</strong>을 가질 수 있습니다.
                    </p>
                </div>
            </div>
        </div>
    </section>

    <section class="section02">
        <div class="max-layout-width">
            <div class="text">
                <div>
                    <h2 class="section-title wow fadeInUp" data-wow-delay="300ms">
                        성장기 성격 형성에 <span class="section-title-color"><strong>긍정적인 효과를<br> 기대</strong></span>할 수 있는 치아교정
                    </h2>
                    <p class="section-text wow fadeInUp" data-wow-delay="600ms">
                        치아에 콤플렉스가 있는 청소년들은 성장기 치아교정으로 사회성과 대인관계 형성에 자신감을 회복 또는 부여할 수 있습니다.
                    </p>
                    <ul>
                        <li class="img01 wow fadeInUp" data-wow-delay="900ms"><p>성인보다<br>치아 이동이 빠름</p></li>
                        <li class="img02 wow fadeInUp" data-wow-delay="1200ms"><p>뼈가 완전히 굳어지지 않아<br>통증이 적음</p></li>
                        <li class="img03 wow fadeInUp" data-wow-delay="1500ms"><p>골격 성장을 조절하여<br>비발치 교정 가능</p></li>
                        <li class="img04 wow fadeInUp" data-wow-delay="1800ms"><p>불규칙한 치열로 인한<br>콤플렉스 해소</p></li>
                    </ul>
                </div>
            </div>
        </div>
    </section>

    <section class="section03 list-content">
        <div class="max-layout-width">
            <div class="image_box_right wow fadeInUp" data-wow-delay="300ms">
                <p class="img"></p>
            </div>
            <h2 class="section-title wow fadeInUp" data-wow-delay="600ms">
                <span class="section-title-color">성장기 교정이</span> 필요한 경우<br>
            </h2>
            <p class="pager active wow fadeInUp" data-wow-delay="900ms"><span></span>한쪽으로 씹는 습관으로 부정교합이 생긴 경우</p><br>
            <p class="pager wow fadeInUp" data-wow-delay="900ms"><span></span>윗니와 아랫니가 반대로 물리는 경우 (주걱턱)</p><br>
            <p class="pager wow fadeInUp" data-wow-delay="900ms"><span></span>아래턱의 성장이 적은 무턱의 증상인 경우</p><br>
            <p class="pager wow fadeInUp" data-wow-delay="900ms"><span></span>선천적 부정교합이 있는 경우 (유전)</p><br>
        </div>
    </section>

    <section class="section04 section-quotes-title section-quotes-title-left">
        <div class="image_box_left wow fadeInUp" data-wow-delay="300ms">
            <p class="img"></p>
        </div>
        <div class="max-layout-width">
            <div class="text">
                <div>
                    <h2 class="section-title wow fadeInUp" data-wow-delay="600ms">
                        성장기 교정, <strong>왜<br>미소원 치과 일까요?</strong>
                    </h2>
                    <p class="section-text wow fadeInUp" data-wow-delay="900ms">
                        성장기는 신체발달과 성장이 왕성하여 교정치료의 효과와 효율이 가장 좋은 시기입니다. 미소원 치과는 정기적인 검진을 통해 골격 교정이 가능한 적정 시기를 놓치지 않습니다.<br>
                        또한,매복 영구치가 바르게 맹출될 수 있도록 유도하며,성장하는 턱의 위치를 교정하는 고난이도 교정이 가능합니다.<br>
                        부정교합의 초기증상이 보이는 경우 구체적인 상담을 권유 드립니다.
                    </p>
                </div>
            </div>
        </div>
        <div class="bg"></div>
    </section>

    <section class="section07 treatment-case wow fadeInUp">
        <div class="max-layout-width">
            <div class="text">
                <h2>미소원치과 <strong>치료사례</strong></h2>
                <p>
                    치료기간 개인 구강상태 및 전신상태등에 따라 3~6개월의 치료기간이 소요됩니다.<br>
                    <span class="pc">
                        개인별로 치료결과가 다를 수 있으며 본 이미지는 실제와 상이할 수 있습니다.
                    </span>
                </p>
            </div>
            <div class="treatment-case-scroll-bar">
                <div class="treatment-case-scroll-bar-drag"></div>
            </div>
            <div class="treatment-case-slider">
                <div class="treatment-case-swiper-slide">
                    <div class="left-img left-img1">
                        <sec:authorize access="isAnonymous()">
                            <div class="login">
                                <a href="<c:url value='/login'/>">
                                    <p>
                                        로그인하시면 Before를 보실 수 있습니다.
                                        <span>로그인하기</span>
                                    </p>
                                </a>
                            </div>
                        </sec:authorize>
                        <div class="text-title">BEFORE</div>
                    </div>
                    <div class="right-img right-img1">
                        <div class="text-title">AFTER</div>
                    </div>
                </div>
                <div class="treatment-case-swiper-slide">
                    <div class="left-img left-img1">
                        <sec:authorize access="isAnonymous()">
                            <div class="login">
                                <a href="<c:url value='/login'/>">
                                    <p>
                                        로그인하시면 Before를 보실 수 있습니다.
                                        <span>로그인하기</span>
                                    </p>
                                </a>
                            </div>
                        </sec:authorize>
                        <div class="text-title">BEFORE</div>
                    </div>
                    <div class="right-img right-img1">
                        <div class="text-title">AFTER</div>
                    </div>
                </div>
                <div class="treatment-case-swiper-slide">
                    <div class="left-img left-img1">
                        <sec:authorize access="isAnonymous()">
                            <div class="login">
                                <a href="<c:url value='/login'/>">
                                    <p>
                                        로그인하시면 Before를 보실 수 있습니다.
                                        <span>로그인하기</span>
                                    </p>
                                </a>
                            </div>
                        </sec:authorize>
                        <div class="text-title">BEFORE</div>
                    </div>
                    <div class="right-img right-img1">
                        <div class="text-title">AFTER</div>
                    </div>
                </div>
            </div>
            <div class="text">
                <p class="mobile">
                    개인별로 치료결과가 다를 수 있으며 본 이미지는 실제와 상이할 수 있습니다.
                </p>
            </div>
        </div>
    </section>

    <section class="board-link">
        <div class="max-layout-width">
            <h2 class="section-title wow fadeInUp" data-wow-delay="300ms">
                학습하고 연구하는 전문 의료진의<br>
                <span class="section-title-color">정직하고 양심적인 진료</span>
            </h2>
            <p class="section-text wow fadeInUp" data-wow-delay="600ms">환자들과 소통하며 신뢰받는 환자중심의 미소원치과가 되고자합니다.</p>

            <ul>
                <li class="wow fadeInUp" data-wow-delay="900ms">
                    <p>
                        <span>온라인 상담</span>
                        <a href="<c:url value='/online-consult/1' /> "><span class="view">VIEW</span></a>
                    </p>
                </li>
                <li class="center-img wow fadeInUp" data-wow-delay="1200ms">
                    <p>
                        <span>치료후기</span>
                        <a href="<c:url value='/review/1' /> "><span class="view">VIEW</span></a>
                    </p>
                </li>
                <li class="last-img wow fadeInUp" data-wow-delay="1500ms">
                    <p>
                        <span>치료사례</span>
                        <a href="<c:url value='/case/1' /> "><span class="view">VIEW</span></a>
                    </p>
                </li>
            </ul>
        </div>
    </section>
</div>

<script src="<c:url value='/lib/jquery-ui/jquery-ui.min.js' />"></script>
<script src="<c:url value="/lib/slick/js/slick.min.js" />"></script>
<script src="<c:url value="/js/cmmn/slider.js" />"></script>


<script>
    $(function () {
        var index = 0;
        $('.list-content .pager').mouseover(function () {
            index = $('.list-content .pager').index($(this));
            active(index);
        })

        setInterval(function () {
            active(index);
        }, 4000);

        function active(_index) {
            $('.list-content .pager').removeClass('active');
            $('.list-content .pager').eq(_index).addClass('active');
            index = (_index + 1) % $('.list-content .pager').length;
        }
    })
</script>