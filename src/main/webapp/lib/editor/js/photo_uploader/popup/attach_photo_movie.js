	...
	var nTotalSize = 0;
	var nMaxImageSize = 10*1024*1024;						//파일 업로드 가능 사이즈
    ...
    function html5Upload() {
    	...
    	sUploadURL= '/util/file_uploader.do'; 	//upload URL
...
// 			sUrl  : location.href.replace(/\/[^\/]*$/, '') + '/file_uploader.php',	//샘플 URL입니다.
 			sUrl  : '/util/fileUploader.do',	//샘플 URL입니다.
 	        sCallback : location.href.replace(/\/[^\/]*$/, '') + '/callback.html',	//업로드 이후에 iframe이 redirect될 콜백페이지의 주소
 	    	sFiletype : "*.jpg;*.png;*.bmp;*.gif;*.mp4",						//허용할 파일의 형식. ex) "*", "*.*", "*.jpg", 구분자(;)	
 	    	sMsgNotAllowedExt : 'JPG, GIF, PNG, BMP, MP4 확장자만 가능합니다',	//허용할 파일의 형식이 아닌경우에 띄워주는 경고창의 문구
...
// 	    		oCustomEvent.stop(); 수행시 bAllowed 가 false이더라도 alert이 수행되지 않음
 	    		...
 	    		//용량 체크
 	    		var img = jindo.$$.getSingle("#uploadInputBox");
 	    		var imgFile = img.files[0];
 	    		var maxVolume = 10;
 	    		var ext = imgFile.type;
 	    		
 	    		if(imgFile == undefined) {
 	    			
 	    			oCustomEvent.sMsgNotAllowedExt;
 	    			return;
 	    		}
 	    		
 	    		if(ext == "video/mp4") {
 	    			
 	    			if((imgFile.size || imgFile.fileSize) / 1024 > (maxVolume * 5 * 1024)) {
 	 	    			
 	 	    			alert("동영상의 용량 제한은 " + maxVolume * 5 + "MB 입니다.");
 	 	    			img.value = "";
 	 	    		}
 	    		} else {
 	    			
 	    			if((imgFile.size || imgFile.fileSize) / 1024 > (maxVolume * 1024)) {
 	 	    			
 	 	    			alert("이미지의 용량 제한은 " + maxVolume + "MB 입니다.");
 	 	    			img.value = "";
 	 	    		}
 	    		}
